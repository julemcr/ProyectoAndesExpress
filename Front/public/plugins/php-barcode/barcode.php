...
function barcode( $filepath="", $text="0", $size="20", $orientation="horizontal", $code_type="code128", $print=false, $SizeFactor=1 ) {
	...
	$image = imagecreate(intval($img_width),intval($img_height + $text_height));
	...
	for ( $position = 1 ; $position <= strlen($code_string); $position++ ) {
		...
		if ( strtolower($orientation) == "horizontal" )
			imagefilledrectangle( $image, intval($location*$SizeFactor), 0, intval($cur_size*$SizeFactor), $img_height, ($position % 2 == 0 ? $white : $black) );
		else
